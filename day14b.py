...
class RecipeBook:
    def __init__(self, data, leftovers):
        self.sourceFormulas = {}
        for d in data:
            f = Formula(d)
    ...
    def getFromLeftovers(self, type, count):
        if type not in self.leftovers:
            return 0

        if self.leftovers[type] >= count:
            self.leftovers[type] -= count
            if self.leftovers[type] == 0:
                del self.leftovers[type]
            return count
        else:  # not enough left
            val = self.leftovers[type]
            self.leftovers[type] = 0
            return val
    def storeLeftovers(self, type, count):
        if type not in self.leftovers:
            self.leftovers[type] = 0
        self.leftovers[type] += count

    def create(self, target, targetCount):
        if target == "ORE":
            return targetCount
        ...
        if produced > targetCount:
            self.storeLeftovers(target, produced - targetCount)

        sum = 0
        for ingredient in recipe.ingredients:
            count = ingredient.count * multi
            type = ingredient.type
            count -= self.getFromLeftovers(type, count)
            if count > 0:
                sum += self.create(type, count)

        return sum

def main():
    leftovers = {"ORE": 1000000000000}
    rb = RecipeBook(data, leftovers)
    print(leftovers)
    print(rb.create("FUEL", 2521844))
