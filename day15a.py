...
class Robot:
    def __init__(self, software):
        self.inputs = [1]
        self.outputs = []
        self.brain = Processor(software, self.inputs, self.outputs)
    ...
    def findNewPos(self):
        pos = self.pos
        step = MOVEMENTS[self.direction]
        return [pos[0] + step[0], pos[1] + step[1]]

    def processResult(self, result):
        nextPos = self.findNewPos()
        x = nextPos[0]
        y = nextPos[1]
        ...
        if result == 0:
            ...
            if result == 1:
                ...
                self.grid.setAt(x, y, '*')
            self.pos = nextPos
        self.inputs.append(self.direction)

    def run(self):
        while True:
            self.brain.runStep(True)
            if len(self.brain.output) == 0:
                continue
            result = self.brain.output.pop(0)
